...
import { BUILTIN, hostedPlugins, type PluginCatalog, type PluginView } from './plugin-catalog';
    ...
    <div data-testid="overlay-menu-plugins" className="flex flex-col gap-1">
      <div className="max-h-48 overflow-y-auto">
        ...
        {catalog.plugins.map(plugin => {
          ...
          return <button key={plugin.manifest.id} type="button" disabled={!compatible || plugin.enabled===false}
            className={overlayRowClass + ' disabled:cursor-not-allowed disabled:opacity-40'}
            aria-pressed={selected === plugin.manifest.id} title={compatible ? plugin.manifest.name : plugin.manifest.name + ' · Available in another editor'}
            onClick={() => open({ kind: 'plugin', id: plugin.manifest.id })}>
            <Puzzle size={14} className="shrink-0 text-neutral-400 dark:text-white/50" />
            <span className="min-w-0 flex-1 truncate">{plugin.manifest.name}</span>
          </button>;
        })}
        {!catalog.plugins.length && (hostedPlugins || tool !== 'pcbnew') && <p className="px-2 py-1 text-xs text-neutral-500 dark:text-white/50">{catalog.loaded ? 'No added plugins yet' : 'Loading plugins…'}</p>}
        {catalog.error && <p role="status" className="px-2 py-1 text-xs text-amber-700 dark:text-amber-200">Plugin list unavailable</p>}
      </div>
      <button type="button" className={overlayRowClass} onClick={() => open({ kind: 'manager' })}>
