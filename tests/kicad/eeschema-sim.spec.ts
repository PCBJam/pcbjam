...
async function runSimulation(page: import('@playwright/test').Page): Promise<number> {
    ...
    // The native receipt is sent from the owner tail after the final refreshes.
    // The barrier additionally proves that the retained dispatch owner retired;
    // it is not used to infer which simulator run finished.
    await page.evaluate(async () => {
        const scheduler = (globalThis as any).__wxScheduler;
        if (!scheduler || typeof scheduler.executionBarrier !== 'function')
            throw new Error('wx execution barrier is missing');
        await scheduler.executionBarrier('ngspice final plot apply');
    });
...
test.describe('eeschema simulator', () => {
    ...
    test('transient run: live console stream, vectors reach the plot, plot renders', async ({ page, testLogger }) => {
        ...
        // The exact final-refresh receipt and owner barrier above prove this
        // log entry belongs to a vector which reached the plot, not merely a
        // worker response still waiting to copy into native memory.
